...
class TaskScheduler:
    ...
    def run_task(
        ...
        """
        self.logger.info(f"Starting task for {type(ir_object).__name__}: {ir_object.name if hasattr(ir_object, 'name') else 'unknown'}")
        ...
        try:
            ...
            if pre_generated_mrs is not None:
                ...
                if self.mr_generator is None:
                    raise ValueError("MR generator not provided and no pre-generated MRs")
