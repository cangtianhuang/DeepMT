...
class TestMRVerifier:
    ...
    def test_scalar_rhs_broadcast(self, verifier, plugin):
        """rhs 为标量时（如 orig * trans == 1）需 broadcast 处理，不应崩溃"""
        x = torch.tensor([1.0, 1.0, 1.0])
        orig = torch.ones(3)
        trans = torch.ones(3)
        result = verifier.verify(orig, trans, _mr("orig * trans == 1"), plugin, x_input=x)
        assert result.passed
    ...
    def test_empty_oracle_expr_treated_as_equality(self, verifier, plugin):
    ...
    def test_all_quantifier_stripped(self, verifier, plugin):
        """all(trans >= orig) 剥离 all() 后等同于 trans >= orig"""
        x = torch.tensor([1.0, 2.0, 3.0])
        orig = F.relu(x)
        trans = F.relu(x + 1)
        result = verifier.verify(orig, trans, _mr("all(trans >= orig)"), plugin, x_input=x)
        assert result.passed
...
class TestOracleDiagnostics:
    ...
    def test_equality_diagnostics_on_pass(self, verifier, plugin):
        """== 通过时：mismatched=0, total>0, max_rel_diff=0"""
        orig = torch.tensor([1.0, 2.0, 3.0])
        result = verifier.verify(orig, orig.clone(), _mr("orig == trans"), plugin)
        assert result.passed
    ...
    def test_equality_diagnostics_on_fail(self, verifier, plugin):
