    ...
    def compare(
        ...
        try:
            ...
            if mr.expected == "equal":
                ...
                is_match, defect_info = self._check_proportional(orig_arr, trans_arr, tolerance)
            elif mr.expected == "invariant":
                is_match, defect_info = self._check_invariant(orig_arr, trans_arr, tolerance)
            elif mr.expected == "monotonic":
                is_match, defect_info = self._check_monotonic(orig_arr, trans_arr, tolerance)
            else:
                # 自定义比对逻辑
                is_match, defect_info = self._check_custom(orig_arr, trans_arr, mr, tolerance)
