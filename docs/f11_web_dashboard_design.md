...
- [x] `pip install -e ".[ui]"` 后 `deepmt ui --port 8080` 正常启动
- [x] 浏览器访问 `http://localhost:8080` 能看到总览页，KPI 卡片有数值
- [x] MR 知识库页能列出所有已保存算子的 MR，筛选功能可用
- [x] 测试结果页能展示 `data/defects.db` 中的历史测试数据
- [x] 跨框架页在有数据时可展示一致率图表，无数据时有友好提示
- [x] 所有 CDN 资源本地化，断网环境下仪表盘功能正常
- [x] `/api/**` 端点返回正确 JSON，可供外部脚本调用
- [x] 新增 API 端点的单元测试（mock 数据源）
