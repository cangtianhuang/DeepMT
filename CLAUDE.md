...
已完成：A1~A6（算子目录与 input_specs）、B1~B3（MR 知识库与批量生成）、C1~C5（RandomGenerator + BatchTestRunner + test batch 命令）、D1~D7（报告生成、变异测试、证据包、跨框架一致性、RQ1-RQ4 数据组织）、F1~F11（pyproject.toml 规范化、依赖完整化、stub 清理、版本导出、API 对齐、文档引用修复、Web 仪表盘）。全量单元测试 369 个通过。
